...
{
    void runWithExpectedOutput(DialogueContent inText);
}
...
public class SpeechSandboxStreaming : MonoBehaviour
{
    ...
    public UnityEngine.Object GameLogicController;
    private dialogFocus _GameLogicController;
    public TextMeshPro textmesh;
    public AudioClip sorryClip;
    public List<AudioClip> helpClips;
    private bool AIStandby = false;

    public static void checkService<T>(T LogicManager)
    {
    ...
    */
    ...
    void OnMessage(DetailedResponse<MessageResponse> resp, IBMError error)
    {
        if (resp != null && resp.Result.Output.Intents.Count  != 0 )
        {
            ...
            DialogueStatus tempSts;
            if (System.Enum.TryParse(intent, out tempSts))
            {

                //Debug.Log("1234");
                //DialogueStatus.initial_Trigger.ToString("G");
                switch (tempSts)
                {
                    ...
                    case DialogueStatus.Display:
                        ...
                        {
                            Debug.Log("5678");
                            //Debug.Log(resp.Result.Output.Entities[0].Value);
                            DialogueContent entity;
                            if (System.Enum.TryParse(resp.Result.Output.Entities[0].Value, out entity))
                            { (GameLogicController as dialogFocus).runWithExpectedOutput(entity); AIStandby = false; }
                        }
                        break;
                    default:
                        //AIStandby = false;
            ...
            textmesh.text = intent+resp.Result.Output.Entities[0].Entity;
